...
type fakeKubernetesCoreClientWithClientSet struct {
	clientSet kubernetes.Interface
}

func (f *fakeKubernetesCoreClientWithClientSet) PodClient(namespace string) coretypedv1.PodInterface {
	return nil
}

func (f *fakeKubernetesCoreClientWithClientSet) GetClientSet() kubernetes.Interface {
	return f.clientSet
}
...
func TestRetryRun_ReopensMLflowParentAndFailedNestedRuns(t *testing.T) {
	...
	originalTokenPath := mlflowServiceAccountTokenPath
	mlflowServiceAccountTokenPath = tokenFile.Name()
	defer func() {
		mlflowServiceAccountTokenPath = originalTokenPath
	}()
	...
	assert.Equal(t, apiv2beta1.RuntimeState_SUCCEEDED, updatedOutput.State)
...
func TestResolveMLflowPluginInput(t *testing.T) {
	largeTextPtr := func(s string) *model.LargeText {
	...
	tests := []struct {
		...
		{
			name: "nil run defaults",
			run:  nil,
			want: &mlflowPluginInput{ExperimentName: defaultMLflowExperimentName},
		},
		{
			name: "nil plugins_input defaults",
			run:  &model.Run{},
			want: &mlflowPluginInput{ExperimentName: defaultMLflowExperimentName},
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"other":{"x":"y"}}`),
				},
			},
			want: &mlflowPluginInput{ExperimentName: defaultMLflowExperimentName},
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"disabled":false}}`),
				},
			},
			want: &mlflowPluginInput{ExperimentName: defaultMLflowExperimentName},
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"experiment_name":"exp-1"}}`),
				},
			},
		...
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"experiment_name":"exp-1","experiment_id":"42"}}`),
				},
			},
		...
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"disabled":true}}`),
				},
			},
		...
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{`),
				},
			},
			wantError: true,
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"experiment_name":123}}`),
				},
			},
			wantError: true,
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"experiment_id":123}}`),
				},
			},
			wantError: true,
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":{"experiment_name":"exp-1","unknown":"x"}}`),
				},
			},
			wantError: true,
		},
		{
			...
			run: &model.Run{
				RunDetails: model.RunDetails{
					PluginsInputString: largeTextPtr(`{"mlflow":"not-an-object"}`),
				},
			},
			wantError: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := resolveMLflowPluginInput(tt.run)
			if tt.wantError {
				require.Error(t, err)
				return
			}
...
func TestSelectMLflowExperiment(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:              nil,
			wantExperimentID:   "",
			wantExperimentName: defaultMLflowExperimentName,
		},
		{
...
func TestGetGlobalMLflowPluginConfig(t *testing.T) {
	...
	cfg, ok, err := getGlobalMLflowPluginConfig()
	require.NoError(t, err)
	require.True(t, ok)
...
func TestAuthorizeMLflowExperimentAction(t *testing.T) {
	newCtxWithUserHeader := func(user string) context.Context {
	...
	t.Run("standalone mode auth disabled skips check", func(t *testing.T) {
		...
		viper.Set(mlflowAuthorizationEnabledConfigKey, "false")
		require.NoError(t, manager.authorizeMLflowExperimentAction(context.Background(), "ns1"))
	})
	...
	t.Run("standalone mode auth enabled requires forwarded user header", func(t *testing.T) {
		...
		defer store.Close()
		viper.Set(common.MultiUserMode, "false")
		viper.Set(mlflowAuthorizationEnabledConfigKey, "true")
		err := manager.authorizeMLflowExperimentAction(context.Background(), "ns1")
		require.Error(t, err)
	...
	t.Run("standalone mode auth enabled fails when SAR denies", func(t *testing.T) {
		...
		viper.Set(common.MultiUserMode, "false")
		viper.Set(mlflowAuthorizationEnabledConfigKey, "true")
		err := manager.authorizeMLflowExperimentAction(newCtxWithUserHeader("user@example.com"), "ns1")
		require.Error(t, err)
...
func TestBuildMLflowRequestContextKubernetesAuth(t *testing.T) {
	...
	requestCfg := &MLflowRequestConfig{
	...
	mlflowCtx, err := manager.buildMLflowRequestContext(context.Background(), "ns1", requestCfg)
	require.NoError(t, err)
...
func TestBuildMLflowRequestContextSecretBasedAuth(t *testing.T) {
	...
	manager.k8sCoreClient = &fakeKubernetesCoreClientWithClientSet{clientSet: clientSet}
	...
	t.Run("basic auth", func(t *testing.T) {
		...
		requestCfg := &MLflowRequestConfig{
			...
			PluginSettings: &MLflowPluginSettings{
				AuthType: mlflowAuthTypeBasicAuth,
				CredentialSecretRef: &CredentialSecretRef{
					Name:        "mlflow-creds",
...
func TestBuildMLflowRequestContextMissingSecretRefValidation(t *testing.T) {
	...
	_, err := manager.buildMLflowRequestContext(context.Background(), "ns1", requestCfg)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "credentialSecretRef.name is required")
}

func TestEnsureMLflowExperimentExists(t *testing.T) {
	store, manager, _ := initWithExperiment(t)
	defer store.Close()

	newSecretBackedRequestContext := func(serverURL string) *MLflowRequestContext {
		clientSet := k8sfake.NewSimpleClientset(
			&corev1.Secret{
				ObjectMeta: v1.ObjectMeta{
					Name:      "mlflow-creds",
	...
	t.Run("returns existing experiment from get-by-name", func(t *testing.T) {
		...
		mlflowCtx := newSecretBackedRequestContext(server.URL)
		exp, err := apiservermlflow.EnsureExperimentExists(context.Background(), toAPIServerMLflowRequestContext(mlflowCtx), "", "my-exp", apiservermlflow.ResolveExperimentDescription(nil))
		require.NoError(t, err)
...
func TestCreateMLflowParentRunAndTagWithKFPMetadata(t *testing.T) {
	store, manager, _ := initWithExperiment(t)
	defer store.Close()
	...
	mlflowRunID, err := apiservermlflow.CreateParentRun(context.Background(), toAPIServerMLflowRequestContext(mlflowCtx), "exp-1", "sample-run")
	require.NoError(t, err)
	...
	err = apiservermlflow.TagRunWithKFPMetadata(context.Background(), toAPIServerMLflowRequestContext(mlflowCtx), mlflowRunID, run)
	require.NoError(t, err)

	require.Len(t, seenTagPayloads, 4)
	assert.Contains(t, seenTagPayloads, setTagRequest{
		RunID: "mlflow-parent-run-1",
		Key:   apiservermlflow.TagKFPRunID,
		Value: "kfp-run-1",
	})
	assert.Contains(t, seenTagPayloads, setTagRequest{
		RunID: "mlflow-parent-run-1",
		Key:   apiservermlflow.TagKFPRunURL,
		Value: "/#/runs/details/kfp-run-1",
	})
	assert.Contains(t, seenTagPayloads, setTagRequest{
		RunID: "mlflow-parent-run-1",
		Key:   apiservermlflow.TagKFPPipelineID,
		Value: "pipeline-1",
	})
	assert.Contains(t, seenTagPayloads, setTagRequest{
		RunID: "mlflow-parent-run-1",
		Key:   apiservermlflow.TagKFPPipelineVersionID,
		Value: "pipeline-version-1",
	})
}

func TestBuildMLflowPluginOutput(t *testing.T) {
	output := apiservermlflow.SuccessfulPluginOutput("exp-1", "my-exp", "run-1", "https://mlflow.example/runs/run-1")
	require.NotNil(t, output)
	assert.Equal(t, apiv2beta1.RuntimeState_SUCCEEDED, output.State)
	require.Contains(t, output.Entries, "run_url")
	require.NotNil(t, output.Entries["run_url"].ContentType)
	assert.Equal(t, apiv2beta1.MetadataValue_URL, *output.Entries["run_url"].ContentType)
}

func TestUpsertRunPluginOutput(t *testing.T) {
	run := &model.Run{
		RunDetails: model.RunDetails{
			PluginsOutputString: testLargeTextPtr(`{"other":{"state":"SUCCEEDED"}}`),
		},
	}
	mlflowOutput := apiservermlflow.SuccessfulPluginOutput("exp-1", "my-exp", "run-1", "https://mlflow.example/runs/run-1")
	err := apiservermlflow.UpsertRunPluginOutput(run, "mlflow", mlflowOutput)
	require.NoError(t, err)
	...
	assert.Equal(t, apiv2beta1.RuntimeState_SUCCEEDED, parsed.State)
