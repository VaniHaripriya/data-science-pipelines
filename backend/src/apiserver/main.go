...
var (
	...
	onlyKubernetesWebhookMode = flag.Bool("globalKubernetesWebhookMode", false, "Runs the API server with only the Kubernetes webhook endpoints enabled")
...
func main() {
	...
	onlyKubernetesWebhookMode := *onlyKubernetesWebhookMode

	clientManager := cm.NewClientManager(onlyKubernetesWebhookMode)
