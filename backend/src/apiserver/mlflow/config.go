...
type PluginSettings struct {
	...
	// KFPBaseURL is the public base URL of the KFP UI (e.g. "https://pipelines.example.com").
	// When set, the kfp.pipeline_run_url MLflow tag is an absolute URL.
	// When empty, a relative path (/#/runs/details/<id>) is used instead.
...
// ---- KubeClientProvider abstraction ----
...
// GetGlobalPluginConfig reads the global plugins.mlflow configuration from
// the API server's viper configuration.
...
func ResolveAuthMaterial(ctx context.Context, clientSet kubernetes.Interface, namespace string, settings *PluginSettings) (AuthMaterial, error) {
	...
	default:
		return AuthMaterial{}, util.NewInvalidInputError("unsupported plugins.mlflow.settings.authType %q (expected one of %q, %q, %q)", authType, DefaultAuthType, AuthTypeBearer, AuthTypeBasicAuth)
	}
}
