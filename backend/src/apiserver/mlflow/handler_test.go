...
func TestOnRunStart_Success(t *testing.T) {
	...
	viper.Set("CommonConfig.MultiUserMode", "false")
	t.Cleanup(func() { viper.Set("CommonConfig.MultiUserMode", "") })
...
func TestOnRunStart_MLflowFailure_ReturnsFailedOutput(t *testing.T) {
	...
	require.Error(t, err)
	require.NotNil(t, output)
	assert.Equal(t, apiv2beta1.RuntimeState_FAILED, output.State)
...
func TestOnRunEnd_MissingRootRunID_PersistsFailed(t *testing.T) {
	...
	outputJSON := `{"mlflow":{"entries":{"experiment_id":{"value":"42"}},"state":"SUCCEEDED"}}`
...
func TestOnRunEnd_Success(t *testing.T) {
	...
	outputJSON := fmt.Sprintf(`{"mlflow":{"entries":{"experiment_id":{"value":"exp-1"},"root_run_id":{"value":"mlflow-parent-1"}},"state":"SUCCEEDED"}}`)
	lt := model.LargeText(outputJSON)
	run := &model.Run{
		UUID: "r-end-1",
...
func TestHandleRetry_Success(t *testing.T) {
	...
	outputJSON := `{"mlflow":{"entries":{"experiment_id":{"value":"exp-1"},"root_run_id":{"value":"parent-1"}},"state":"FAILED","stateMessage":"previous failure"}}`
	lt := model.LargeText(outputJSON)
	run := &model.Run{
		UUID:       "r-retry-ok",
		RunDetails: model.RunDetails{PluginsOutputString: &lt},
