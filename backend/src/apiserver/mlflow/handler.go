...
func (h *Handler) syncOnRunTerminal(ctx context.Context, run *model.Run, namespace string) error {
	...
	if parentRunID == "" {
		SetPluginOutputState(pluginOutput, apiv2beta1.RuntimeState_FAILED, "MLflow terminal sync skipped: missing parent root_run_id in plugins_output.mlflow")
	...
	if cfgErr != nil || requestCfg == nil {
		...
		SetPluginOutputState(pluginOutput, apiv2beta1.RuntimeState_FAILED, message)
		if upsertErr := UpsertRunPluginOutput(run, PluginName, pluginOutput); upsertErr != nil {
			return upsertErr
		}
		return h.deps.RunStoreUpdater.UpdateRunPluginsOutput(run.UUID, run.PluginsOutputString)
	...
	if len(syncErrors) > 0 {
		SetPluginOutputState(pluginOutput, apiv2beta1.RuntimeState_FAILED, strings.Join(syncErrors, "; "))
	} else {
		SetPluginOutputState(pluginOutput, apiv2beta1.RuntimeState_SUCCEEDED, "")
	}
...
func (h *Handler) HandleRetry(ctx context.Context, run *model.Run, namespace string) {
	...
	experimentID := GetStringEntry(pluginOutput, EntryExperimentID)
	if parentRunID == "" {
		SetPluginOutputState(pluginOutput, apiv2beta1.RuntimeState_FAILED, "MLflow retry sync skipped: missing parent root_run_id in plugins_output.mlflow")
