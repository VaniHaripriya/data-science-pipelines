...
func TestCreateRunWithPluginsFields(t *testing.T) {
	...
	run := &model.Run{
		...
		RunDetails: model.RunDetails{
			...
			PluginsOutputString:     testLargeTextPtr(`{"mlflow":{"entries":{"root_run_id":{"value":"abc123"}},"state":"SUCCEEDED","stateMessage":"ok"}}`),
		},
	}
	_, err := runStore.CreateRun(run)
	...
	assert.Equal(t, model.LargeText(`{"mlflow":{"entries":{"root_run_id":{"value":"abc123"}},"state":"SUCCEEDED","stateMessage":"ok"}}`), *got.PluginsOutputString)
}
...
func TestUpdateRunPreservesPluginsFields(t *testing.T) {
	...
	require.NotNil(t, got.PluginsOutputString)
	assert.Equal(t, model.LargeText(`{"mlflow":{"state":"RUNNING"}}`), *got.PluginsOutputString)
...
func TestUpdateRunPluginsOutputOnly(t *testing.T) {
	...
	updatedOutput := testLargeTextPtr(`{"mlflow":{"state":"SUCCEEDED","stateMessage":""}}`)
	...
	require.NotNil(t, got.PluginsOutputString)
	assert.Equal(t, model.LargeText(`{"mlflow":{"state":"SUCCEEDED","stateMessage":""}}`), *got.PluginsOutputString)
}
...
func TestUpdateRunPluginsOutputNotFound(t *testing.T) {
	...
	output := testLargeTextPtr(`{"mlflow":{"state":"FAILED"}}`)
...
func TestListRunsReturnsPluginsFields(t *testing.T) {
	...
	assert.Equal(t, model.LargeText(`{"mlflow":{"state":"RUNNING"}}`), *runs[0].PluginsOutputString)
}
